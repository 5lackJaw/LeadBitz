...
export async function POST(request: Request) {
  ...
  try {
    const result = await completeSpecialistInterviewSession({
    ...
    return NextResponse.json(
      {
        icpVersionId: result.icpVersionId,
    ...
    return NextResponse.json({ error: "Failed to complete specialist interview." }, { status: 500 });
  }
}
